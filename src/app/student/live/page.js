...
import { Search, Radio, ChevronLeft, Lock, Unlock, CreditCard, X, Copy, Building2, MapPin, Home } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';

export default function LiveStreamLibrary() {
    const { user } = useAuth();
    ...
    const load = async () => {
        if (!user) return;
        try {
            const res = await axios.get('/student/content', { params: { batch: user.batch } });
            setAllLessons(res.data.lessons.filter(l => l.type === 'Live'));
    ...
    useEffect(() => {
        if (user) {
            const pageName = activeFolder ? `Browsing: ${activeFolder.name}` : "Live Stream Library";
            emitStudentActivity(socket, user, {
                page: pageName,
                action: activeFolder ? 'Browsing Live Folder' : 'Browsing Live Stream Library',
                detail: activeFolder?.name || 'Root',
                route: '/student/live',
                kind: 'navigation'
            });
        }
    ...
    const handleFolderClick = (lesson) => {
        ...
        if (isUnlocked) {
            emitStudentActivity(socket, user, {
                page: 'Live Class Library',
                action: 'Opened Live Folder',
                detail: lesson.name,
                route: '/student/live',
                kind: 'navigation'
            });
    ...
    const handleHome = () => router.push('/student/live'); 
    const handleCopy = (text) => { navigator.clipboard.writeText(text); alert("Copied: " + text); };
                ...
                >
                    ...
                    >
                        ...
                        <div className="p-4 bg-slate-50 dark:bg-slate-900 border-t border-slate-100 dark:border-white/5 text-center transition-colors duration-300">
                            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Please upload slip after transfer</p>
                        </div>
                    </motion.div>
                </motion.div>
            )}
    ...
    return (
        ...
        <div className="flex flex-col h-full overflow-hidden bg-[#FFFBFB] dark:bg-slate-950 relative pt-4 transition-colors duration-300">
            ...
            <motion.button 
                ...
                className="absolute top-4 right-4 z-50 flex items-center gap-2 px-4 py-2 bg-white dark:bg-slate-900 border border-red-100 dark:border-red-900/50 text-red-600 dark:text-red-500 rounded-full shadow-sm hover:shadow-md hover:border-red-200 dark:hover:border-red-500/50 transition-all"
            >
                <CreditCard size={16} />
            ...
            {/* HEADER AREA */}
            <div className="shrink-0 pb-6 px-4 z-10 mt-2 sm:mt-0">
                <div className="text-center">
                    {/* 🚀 Dark Mode: Title */}
                    <h2 className="text-2xl font-black text-slate-800 dark:text-white uppercase italic tracking-tighter mb-4 pt-8 sm:pt-0 flex items-center justify-center gap-2 transition-colors">
                        <Radio className="text-red-600 animate-pulse" /> Live Stream Library
                    </h2>
                </div>
                
                {/* BREADCRUMBS */}
                <div className="flex items-center gap-3 mb-4 overflow-x-auto max-w-full pb-2 justify-center">
                    {activeFolder && (
                ...
                <div className="max-w-md mx-auto relative group">
                    <Search className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500 group-focus-within:text-red-600 transition-colors" size={20} />
                    {/* 🚀 Dark Mode: Search Bar */}
                    <input value={search} onChange={(e) => setSearch(e.target.value)} type="text" placeholder="Search live sessions..." className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 dark:text-white py-3 pl-14 pr-6 rounded-full shadow-sm outline-none font-bold text-sm focus:border-red-200 dark:focus:border-red-500/50 focus:ring-4 focus:ring-red-50 dark:focus:ring-red-500/10 transition-all placeholder:text-slate-400 dark:placeholder:text-slate-500" />
                </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-4 pb-20 custom-scrollbar">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                        {viewFolders.map(l => {
                            ...
                            const itemCount = isRootFolder
                                ...
                                : allContent.filter(c => c.lessonId === l.id).length; 
                            const isFree = !l.price || l.price === "0";
                            const isUnlocked = l.isPaid || isFree;
                            return (
                                // 🚀 Dark Mode: Folder Cards
                                <div key={l.id} onClick={() => handleFolderClick(l)} className={`relative p-8 rounded-[35px] border shadow-sm transition-all duration-300 cursor-pointer group overflow-hidden ${isUnlocked ? 'bg-white dark:bg-slate-900 border-slate-100 dark:border-white/10 hover:shadow-xl' : 'bg-slate-50 dark:bg-slate-900/50 border-slate-200 dark:border-white/5 hover:border-red-300 dark:hover:border-red-500/50 opacity-90 hover:opacity-100'}`}>
                                    <div className="absolute top-4 right-4">
                                        {isUnlocked ? <div className="bg-green-100 dark:bg-green-500/20 text-green-600 dark:text-green-400 p-2 rounded-full shadow-sm transition-colors"><Unlock size={16} strokeWidth={3} /></div> : <div className="bg-red-100 dark:bg-red-500/20 text-red-600 dark:text-red-400 p-2 rounded-full shadow-sm transition-colors"><Lock size={16} strokeWidth={3} /></div>}
                                    </div>
                                    <div className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-6 transition-all group-hover:scale-110 ${isUnlocked ? 'bg-red-50 dark:bg-red-500/10 text-red-600 dark:text-red-500' : 'bg-slate-200 dark:bg-slate-800 text-slate-400 dark:text-slate-500'}`}><Radio size={28} /></div>
                                    ...
                                    {/* 🚀 BUG FIX: Removed 'truncate' and 'line-clamp-1' so text wraps naturally */}
                                    <h3 className={`font-black text-xl uppercase italic tracking-tighter mb-2 transition-colors break-words whitespace-normal ${isUnlocked ? 'text-slate-800 dark:text-white' : 'text-slate-500 dark:text-slate-400'}`}>
                                        {l.name}
                                    </h3>

                                    <div className="flex justify-between items-center mt-4">
                                        <p className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest transition-colors">{isRootFolder ? `${itemCount} Folders` : `${itemCount} Streams`}</p>
                                        <p className={`text-[10px] font-black uppercase tracking-widest px-2 py-1 rounded transition-colors ${isUnlocked ? 'bg-green-50 dark:bg-green-500/10 text-green-600 dark:text-green-400' : 'bg-red-600 text-white shadow-lg shadow-red-600/20'}`}>{isUnlocked ? 'OPEN' : `LKR ${l.price}`}</p>
                                    </div>
                                </div>
                            );
                        })}
                ...
                {viewVideos.length > 0 && (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {viewVideos.map(item => (
                            <VideoCard key={item.id} item={item} user={user} onUnlock={() => item.isSeparate && setSelectedItem(item)} />
                        ))}
                    </div>
                )}
                ...
                {viewFolders.length === 0 && viewVideos.length === 0 && (
                    ...
                    <div className="flex flex-col items-center justify-center h-64 text-slate-300 dark:text-slate-600 transition-colors">
                        ...
                        <p className="font-black uppercase tracking-widest">No Live Sessions</p>
                    </div>
                )}
            </div>
