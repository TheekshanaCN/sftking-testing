...
function resolvePdfAbsPath(storedPathOrUrl) {
  ...
  const rel = storedPathOrUrl.startsWith("/") ? storedPathOrUrl.slice(1) : storedPathOrUrl;
...
const getFirebaseAdminApp = () => {
  ...
  const existingApp = getFirebaseAdminApps().find((app) => app.name === '[DEFAULT]');
  if (existingApp) {
...
const parseNotificationTokens = (value) => {
  ...
  try {
    ...
    return Array.from(new Set(parsed.map((token) => String(token).trim()).filter(Boolean)));
  } catch {
    return [];
  }
};
...
const removeNotificationToken = async (userId, token) => {
  ...
  const tokens = parseNotificationTokens(user.notificationTokens).filter((item) => item !== token);
  await writeNotificationTokens(userId, tokens);
  return true;
};
...
const setNotificationFlags = ({ emailEnabled, pushEnabled }) => {
  if (typeof emailEnabled === "boolean") notificationsEmailEnabled = emailEnabled;
...
const sendPushNotificationToUsers = async (userIds, notification) => {
  if (!isPushNotificationsEnabled()) return { sent: 0, skipped: true, disabled: true };
  ...
  const tokens = Array.from(new Set(users.flatMap((user) => parseNotificationTokens(user.notificationTokens))));
...
const resolveSupportEmailRecipient = (receiver) => {
  const fallback = process.env.SUPPORT_ALERT_EMAIL || process.env.SMTP_USER || process.env.GMAIL_USER || null;
...
const sendSupportEmailAlert = async ({ sender, receiver, content, messageId }) => {
  console.log('[EMAIL] sendSupportEmailAlert called', { senderId: sender?.id, receiverId: receiver?.id });
  ...
  const senderName = sender?.name || sender?.mobile || `User #${sender?.id || "unknown"}`;
  const supportAddress = process.env.SMTP_USER || process.env.GMAIL_USER || "";
  const portalUrl = receiver?.role === "admin"
  ...
  const preview = String(content || "").trim().slice(0, 72) || "New support message";
...
const parseBatchTargets = (rawBatches) => {
  try {
    const parsed = Array.isArray(rawBatches)
      ? rawBatches
      : (typeof rawBatches === 'string' ? JSON.parse(rawBatches) : []);
...
const sendMcqScheduledNotifications = async ({ quiz, trigger = 'scheduled' }) => {
  ...
  const recipients = students.filter((student) => {
    ...
    const normalizedBatch = String(student.batch || '').trim().toLowerCase();
    return normalizedBatch && targetBatches.includes(normalizedBatch);
  });

  if (!recipients.length) {
    console.log('[MCQ NOTIFY] No recipients for scheduled exam', { quizId: quiz.id, batches: targetBatches });
  ...
  const transporter = getMailTransporter();
  const from = process.env.EMAIL_FROM || process.env.SMTP_USER || process.env.GMAIL_USER;
  const supportAddress = process.env.SMTP_USER || process.env.GMAIL_USER || undefined;
  const examListUrl = process.env.STUDENT_MCQ_LIST_URL || 'https://sftking.lk/student/exams';
  const examBaseUrl = (process.env.STUDENT_MCQ_EXAM_URL_BASE || 'https://sftking.lk/mcq-exam').replace(/\/$/, '');
  const examUrl = `${examBaseUrl}/${quiz.id}`;

  let emailSent = 0;
  if (transporter && from) {
    const emailTargets = recipients.filter((student) => String(student.email || '').trim());
...
const getStudentDisplayId = (student) => {
  ...
  return String(student.studentCode || student.hallClass || `${student.classMode || 'student'}-${student.id || 'na'}`).trim() || 'N/A';
};
...
const sendNewAccountCreatedAdminAlert = async ({ student }) => {
  ...
  const emailTargets = Array.from(new Set([
    ...
    ...adminUsers.map((admin) => String(admin.email || '').trim().toLowerCase()),
  ].filter((value) => isValidEmail(String(value || '').trim().toLowerCase()))));
  ...
  const studentHallClass = student.hallClass ? String(student.hallClass) : 'N/A';
  ...
  const createdAtLabel = student.createdAt
    ? new Date(student.createdAt).toLocaleString('en-GB', { timeZone: 'Asia/Colombo' })
  ...
  try {
    ...
    console.error('[ADMIN NOTIFY] Registration push error:', error?.message || error);
  }
  ...
  if (transporter && from && emailTargets.length) {
    ...
    const emailJobs = emailTargets.map((to) => transporter.sendMail({
      from,
      to,
      subject: 'New account created',
      text,
      html,
      replyTo: process.env.ADMIN_SUPPORT_EMAIL || process.env.SUPPORT_ALERT_EMAIL || undefined,
      headers: {
    ...
    results.forEach((result, idx) => {
      if (result.status === 'fulfilled') {
        ...
        console.error(`[ADMIN NOTIFY] Registration email FAILED to ${emailTargets[idx]}:`, result.reason?.message || result.reason || 'Unknown error');
      }
    });
  } else {
    console.warn('[ADMIN NOTIFY] Registration email skipped due to missing config/targets', {
...
const sendNewAccountCreatedStudentAlert = async ({ student }) => {
  ...
  const displayId = getStudentDisplayId(student);
  const createdAtLabel = formatSriLankaDateTime(student.createdAt || new Date());
  const supportUrl = process.env.STUDENT_SUPPORT_URL || 'https://sftking.lk/student/help';
  const dashboardUrl = process.env.STUDENT_DASHBOARD_URL || 'https://sftking.lk/student/dashboard';
  ...
  try {
    ...
    console.error('[STUDENT NOTIFY] New account push error:', error?.message || error);
  }
  ...
  if (transporter && from && to && isValidEmail(to)) {
    ...
    try {
      await transporter.sendMail({
        ...
        text,
        html,
        replyTo: process.env.SUPPORT_ALERT_EMAIL || process.env.SMTP_USER || process.env.GMAIL_USER || undefined,
        headers: {
      ...
      console.error(`[STUDENT NOTIFY] Welcome email FAILED to ${to}:`, error?.message || error);
      emailSent = 0;
    }
  }
...
const sendAccountStatusAlertToStudent = async ({ user, eventType = 'suspended', reason = '' }) => {
  ...
  const eventLabel = eventType === 'banned'
    ? 'Banned'
    : (eventType === 'reactivated' ? 'Reactivated' : 'Suspended');
  const subject = `Account ${eventLabel}`;
  const effectiveAt = new Date().toLocaleString('en-GB', { timeZone: 'Asia/Colombo' });
  const supportUrl = process.env.STUDENT_SUPPORT_URL || 'https://sftking.lk/student/help';
  const safeReason = String(reason || '').trim() || (eventType === 'banned'
  ...
  const pushPayload = {
    title: eventType === 'banned'
      ? 'Account banned'
      : (eventType === 'reactivated' ? 'Account reactivated' : 'Account suspended'),
    body: eventType === 'banned'
...
const sendRequestSubmissionAlertToAdmins = async ({ request, student, lessonName, contentTitle }) => {
  if (!request || !student) return { skipped: true };

  const adminRows = await User.findAll({ where: { role: 'admin' }, attributes: ['id', 'notificationTokens'] });
  const adminIds = adminRows.map((admin) => admin.id);
  const adminTokenCount = Array.from(new Set(adminRows.flatMap((admin) => parseNotificationTokens(admin.notificationTokens)))).length;
  ...
  const studentName = String(student.name || request.studentName || 'Unknown Student');
  ...
  if (transporter && from && adminEmails.length) {
    ...
    const results = await Promise.allSettled(adminEmails.map((to) => transporter.sendMail({
      from,
      to,
      subject,
      text,
    ...
    results.forEach((result, idx) => {
      if (result.status === 'fulfilled') {
        emailSent++;
        console.log(`[REQUEST ALERT] Email sent successfully to ${adminEmails[idx]}`);
      } else {
        const errMsg = result.reason?.message || String(result.reason || 'Unknown error');
        console.error(`[REQUEST ALERT] Email FAILED to ${adminEmails[idx]}: ${errMsg}`);
...
const sendRequestDecisionAlertToStudent = async ({ request, student, status }) => {
  if (!request || !student) return { skipped: true };
  const normalizedStatus = status === 'rejected' ? 'declined' : status;
  if (!['approved', 'declined'].includes(normalizedStatus)) return { skipped: true };
  ...
  const titleLabel = String(request.contentTitle || request.lessonName || 'Your request');
  const supportUrl = process.env.STUDENT_SUPPORT_URL || 'https://sftking.lk/student/help';
  const displayId = getStudentDisplayId(student);
  const batchLabel = String(student.batch || 'N/A');
  const modeLabel = String(student.classMode || 'N/A');
  ...
  try {
    ...
    console.error('[REQUEST ALERT] Student push error:', error?.message || error);
  }
...
server.use('/uploads/written-answers', express.static(path.join(__dirname, 'uploads/written-answers')));
...
const User = sequelize.define("User", {
  ...
  notificationTokens: { type: DataTypes.TEXT, allowNull: true, defaultValue: "[]" },
  welcomeNotificationEligible: { type: DataTypes.BOOLEAN, allowNull: false, defaultValue: false },
  welcomeNotificationSent: { type: DataTypes.BOOLEAN, allowNull: false, defaultValue: false },
});
...
const Content = sequelize.define("Content", {
  ...
  audienceMode: { type: DataTypes.STRING, allowNull: false, defaultValue: "all" },
...
const McqResultHistory = sequelize.define('mcq_result_history', {
  ...
  resetReason: { type: DataTypes.STRING, allowNull: false, defaultValue: 'rescheduled' },
...
const McqAttempt = sequelize.define('mcq_attempt', {
  studentId: { type: DataTypes.INTEGER, allowNull: false },
  quizId: { type: DataTypes.INTEGER, allowNull: false }
}, {
  indexes: [
...
const WrittenQuiz = sequelize.define('written_quiz', {
  ...
  pageImages: { type: DataTypes.TEXT, allowNull: true, defaultValue: '[]' }, // 🚀 THE NEW IMAGE MATRIX
  status: { type: DataTypes.ENUM('scheduled', 'live', 'ended'), defaultValue: 'live' }
});
...
// ✅ 3. RELATIONSHIPS
PastPaperFolder.hasMany(PastPaperFolder, { as: 'SubFolders', foreignKey: 'parentId' });
  ...
  const ensureUniqueIndex = async (indexName, fieldName) => {
    try {
      ...
      const exists = indexes.some((idx) => {
        ...
        return idx.fields.some((f) => f?.attribute === fieldName || f?.name === fieldName);
      });
      ...
      console.log(`[MIGRATE] Index ${indexName} already exists or cannot be created`);
    }
  };
...
const initDB = async () => {
  ...
  const defaultRules = [
    { key: 'sqli_xss_filter', name: 'SQLi & XSS Shredder', description: 'Blocks malicious database injection and script payloads.' },
    { key: 'dir_traversal', name: 'Directory Traversal Blocker', description: 'Prevents hackers from accessing VPS root folders.' },
    { key: 'ddos_ratelimit', name: 'Layer 7 Anti-DDoS', description: 'Drops connections from IPs sending abnormal amounts of traffic.' },
    { key: 'geo_block', name: 'Geo-Fence (LK Only)', description: 'Drops all traffic originating outside of Sri Lanka.' },
    { key: 'vpn_block', name: 'VPN & Proxy Blocker', description: 'Blocks known VPN exit nodes to prevent account sharing.' }
  ];
...
const removeOnlineSession = async (userId, socketId = null) => {
  ...
  const stillOnline = Array.from(onlineUsers.values()).some((info) => String(info.userId) === String(userId));
  if (!stillOnline) {
    await User.update({ isOnline: false }, { where: { id: userId } });
  }

  io.emit("online_count_update", onlineUsers.size);
  io.to("admin_room").emit("active_sessions_update", Array.from(onlineUsers.entries()).map(([id, info]) => ({
...
setInterval(async () => {
  ...
  for (const [userId, info] of onlineUsers.entries()) {
    ...
    if (info.userId !== undefined && lastSeen && now - lastSeen > SESSION_OFFLINE_MS) {
...
io.on("connection", (socket) => {
  ...
  socket.on("i_am_here", async (data) => {
    ...
    if (data.role === "student") {
      ...
      const sessionId = String(data.sessionId || data.sid || `${userId}:${socket.id}`);
      ...
      if (existingUser && existingUser.socketId && existingUser.socketId !== socket.id) {
        try {
          io.to(existingUser.socketId).emit('concurrent_login', {
            message: 'Security Alert: Your account is active on another device. This session has been closed.'
          });
        } catch (error) {
          console.error('[SESSION REPLACE ERROR] Failed to notify old session:', error);
        }

        onlineUsers.delete(userId);
      }

      const currentPage = data.page || ((existingUser && existingUser.page && existingUser.page !== "Logging in...")
        ? existingUser.page
        : "Browsing Platform");
      const currentAction = data.action || ((existingUser && existingUser.action)
        ? existingUser.action
        : currentPage);
      const currentDetail = data.detail || (existingUser && existingUser.detail) || '';
      const currentRoute = data.route || (existingUser && existingUser.route) || null;
      const currentOS = data.os || (existingUser && existingUser.os) || 'Unknown';
      const currentAvatar = data.avatar || (existingUser && existingUser.avatar) || null;

      // ✅ This strictly forces the correct name or falls back to their ID (No more "undefined")
      const finalName = data.name || (existingUser && existingUser.name) || `Student #${userId}`;
  ...
  socket.on("student_activity", (data) => {
    ...
    console.log(`📡 Radar Ping Received from ${safeUserId}: ${action} :: ${page}`);
  ...
  socket.on("mark_read", async ({ senderId, receiverId }) => {
    try {
      ...
      io.to(`user_${receiverId}`).emit("messages_read", { senderId, receiverId });
    } catch (error) {
  ...
  socket.on("send_god_alert", async (data) => {
    try {
      ...
      const priority = ["normal", "high", "critical"].includes(priorityRaw) ? priorityRaw : "high";
      ...
      if (message.length > 320) {
        socket.emit("god_alert_error", { message: "Message too long (max 320 chars)" });
        return;
      }
      ...
      if (!admin || admin.role !== "admin") {
        socket.emit("god_alert_error", { message: "Unauthorized alert sender" });
        return;
      }
  ...
  socket.on("send_message", async (data) => {
    try {
        ...
        include: [
          {
            model: Message,
            as: "replyTo",
            include: [{ model: User, as: "sender", attributes: ["id", "name", "mobile", "role"] }],
          },
        ],
      });
      ...
      console.log('[SUPPORT] Message sent', { senderId, receiverId: recipientUser?.id, recipientIsAway, recipientExists: !!recipientUser });

      if (recipientUser && String(recipientUser.id) !== String(senderId) && recipientIsAway) {
        console.log('[SUPPORT] Recipient is away - triggering push + email');
        const notificationTitle = sender?.role === 'admin' ? 'Support reply from admin' : 'New support message';
        const notificationBody = `${sender?.name || sender?.mobile || 'Someone'}: ${String(content || '').slice(0, 120)}`;
        const notificationUrl = sender?.role === 'admin'
        ...
        console.log('[SUPPORT] Recipient NOT away or not found - skipping push + email');
      }
    } catch (e) {
  ...
  socket.on("disconnect", async () => {
    ...
    if (removedUserIds.length > 0) {
      for (const userId of removedUserIds) {
        const stillOnline = Array.from(onlineUsers.values()).some((info) => String(info.userId) === userId);
        if (!stillOnline) {
          await User.update({ isOnline: false }, { where: { id: userId } });
        }
      }
...
const verifyToken = (req, res, next) => {
  ...
  jwt.verify(
    ...
    async (err, decoded) => {
      ...
      if (decoded.role === "student" && decoded.fp && decoded.fp !== "unknown") {
        if (decoded.fp !== incomingFp) {
          await SecurityLog.create({
            event: "TOKEN HIJACK ATTEMPT",
            description: "Device signature mismatch! Token was moved to a new device.",
            ip_address: req.ip || req.connection.remoteAddress,
            user_id: String(decoded.id),
            severity: "high"
          });
          return res.status(403).json({ message: "Security Violation: Device fingerprint mismatch. Please log in again." });
        }
      }
      ...
      try {
        ...
        if (user.status === "deactivated") return res.status(403).json({ message: "Banned" });
...
const ddosLimiter = rateLimit({
  ...
  message: { error: "SFT FIREWALL: DDoS Protection Triggered. Your IP has been temporarily blocked." },
...
server.use(async (req, res, next) => {
  ...
  const payloadString = JSON.stringify(req.body) + JSON.stringify(req.query) + req.path;

  // 3. Directory Traversal Blocker
  if (firewallCache['dir_traversal'] && (payloadString.includes('../') || payloadString.includes('..\\'))) {
    await SecurityLog.create({ event: "FIREWALL BLOCK", description: "Directory Traversal Attempt", ip_address: cleanIp, severity: "high" });
    return res.status(403).json({ error: "SFT FIREWALL: Directory Traversal Blocked." });
  }

  // 4. SQL Injection & XSS Payload Shredder
  if (firewallCache['sqli_xss_filter']) {
    const sqliXssRegex = /(<script>)|(%3Cscript%3E)|(javascript:)|(\bDROP\b\s+\bTABLE\b)|(\bUNION\b\s+\bSELECT\b)/i;

    if (sqliXssRegex.test(payloadString)) {
      await SecurityLog.create({ event: "FIREWALL BLOCK", description: "SQLi/XSS Malicious Payload Detected", ip_address: cleanIp, severity: "high" });
      return res.status(403).json({ error: "SFT FIREWALL: Malicious Payload Shredded." });
    }
  }
...
app.prepare().then(() => {
  ...
  server.get("/api/content/:id/pdf-token", verifyToken, async (req, res) => {
    ...
    try {
      ...
      if (!content || !content.pdfFile || !content.pdfVisible) {
        return res.status(404).json({ message: "PDF is currently unavailable." });
      }
      ...
      if (!approvedRequest) {
        return res.status(403).json({ message: "Time limit has expired or access denied." });
      }
    ...
    try {
      ...
      jwt.verify(token, process.env.JWT_SECRET || "secret", async (err, decoded) => {
        if (err || decoded.purpose !== "secure_pdf_view") {
          return res.status(403).send("Security Token Expired. Refresh page.");
        }
        ...
        if (isPastPaper) {
          ...
          if (!pastPaper || !pastPaper.pdfFile) return res.status(404).send("Past Paper Not Found");
          targetPdfFile = pastPaper.pdfFile;
        } else {
          const content = await Content.findByPk(contentId);
          if (!content || !content.pdfFile) return res.status(404).send("PDF Not Found");
        ...
        if (!fs.existsSync(pdfPath)) return res.status(404).send("File Missing on Server");
        ...
        res.setHeader("Content-Disposition", `inline; filename="SECURE_DOC.pdf"`);
        ...
        if (range) {
          ...
          const stream = fs.createReadStream(pdfPath, { start: chunkStart, end: chunkEnd });
  ...
  server.get("/api/student/pdf-status/:contentId", verifyToken, async (req, res) => {
    try {
  ...
  server.post("/api/send-otp", async (req, res) => {
    try {
      ...
      await sendTextItSMS(intlMobile, `Dear Student Your One Time Password is ${otp}`);

      return res.json({ success: true, sent: true });
    } catch (e) {
      console.error("send-otp error:", e?.message || e);
      // Send gateway error to frontend for easier debugging
      return res.status(500).json({ message: e?.message || "OTP send failed", sent: false });
    }
  });
  ...
  server.post("/api/user/change-password/verify", verifyToken, async (req, res) => {
    try {
      ...
      return res.status(500).json({ message: "Failed to verify old password" });
    }
  });
  ...
  server.post("/api/me/address", verifyToken, async (req, res) => {
    try {
      ...
      if (/^0\d{9,14}$/.test(address.replace(/\s+/g, ''))) {
        return res.status(400).json({ message: "Enter a real address, not a phone number" });
      }
      ...
      return res.status(500).json({ message: e?.message || "Failed to update address" });
    }
  });

  // 🔧 ADMIN BULK-FIX: Clear bad addresses (phone-number-only entries)
  server.post('/api/admin/fix-bad-addresses', requireAdmin, async (req, res) => {
    try {
      ...
      if (updateCount > 0) {
        await Promise.all(
          badAddresses.map((u) => u.update({ address: '' }))
        );
      }
      ...
      console.error('[ADMIN FIX] Bulk address clear failed:', e?.message || e);
  ...
  server.post("/api/notifications/register-token", verifyToken, async (req, res) => {
    try {
      const token = String(req.body?.token || "").trim();
      console.log('[TOKEN REGISTER] Request received for userId:', req.user.id, 'token length:', token.length);
      ...
      if (!tokens.includes(token)) {
        ...
        console.log('[TOKEN REGISTER] New token added. Total tokens now:', tokens.length);
      } else {
  ...
  server.post("/api/notifications/unregister-token", verifyToken, async (req, res) => {
    try {
      const token = String(req.body?.token || "").trim();
  ...
  server.get("/api/admin/conversations", verifyToken, async (req, res) => {
    try {
      ...
      for (const m of messages) {
        ...
        if (m.sender.id === student.id && !m.isRead && Number(m.receiverId) === Number(myId)) {
  ...
  server.post("/api/messages/clear", verifyToken, async (req, res) => {
    try {
      ...
      const me = await User.findByPk(myId, { attributes: ["id", "role"] });
      if (me?.role === "admin") {
        io.to("admin_room").emit("message_deleted_bulk", { senderId: myId, receiverId: targetId });
      } else {
        io.to(`user_${myId}`).emit("message_deleted_bulk", { senderId: myId, receiverId: targetId });
      }
  ...
  const normalizeLiveSchedulePayload = (payload) => {
    ...
    if (!scheduleEnabled) {
      return { scheduleEnabled: false, startTime: null, endTime: null, shouldBeLive: false };
    }
  ...
  const normalizeLiveLockPayload = (payload, scheduleEnabled, existing = null) => {
    if (!scheduleEnabled) {
      return { lockAfterEnd: false, lockPrice: null };
    }

    const lockAfterEnd = toBoolean(payload?.lockAfterEnd ?? existing?.lockAfterEnd ?? false);
  ...
  const filterRecipientsByAudience = (students, content, options = {}) => {
    ...
    if (mode === "online") {
      list = list.filter((student) => String(student.classMode || "").trim().toLowerCase() === "online");
    }

    if (mode === "city") {
      const targetCity = city.toLowerCase();
      list = list.filter((student) => {
        const studentMode = String(student.classMode || "").trim().toLowerCase();
        const hallClass = String(student.hallClass || "").trim().toLowerCase();
  ...
  const resolveLiveLabel = async (content) => {
    try {
      ...
      if (content.lessonId) {
        ...
        const label = String(lesson?.name || content.title || "Live session").trim();
  ...
  const notifyLiveVideoStarted = async (content, batchNames, options = {}) => {
    try {
      ...
      const recipients = filterRecipientsByAudience(students, content, { onlyOffline });
      ...
      const emails = recipients.map(s => String(s.email || '').trim().toLowerCase()).filter(isValidEmail);
      if (emails.length > 0) {
        const transporter = getMailTransporter();
        const from = process.env.EMAIL_FROM || process.env.SMTP_USER || process.env.GMAIL_USER;
        const html = `
        ...
        emails.forEach(to => {
          ...
          }).catch(err => console.error("Live video email failed for:", to, err));
        });
      }
    } catch (e) {
    ...
    try {
      ...
      if (emails.length > 0) {
        ...
        emails.forEach(to => {
          ...
          }).catch(err => console.error("Live video schedule email failed for:", to, err));
        });
      }
    } catch (e) {
  ...
  setInterval(async () => {
    try {
      ...
      for (const content of scheduled) {
        const shouldBeLive = isWithinScheduleWindow(content.startTime, content.endTime, nowMs);
        const endMs = content.endTime ? new Date(content.endTime).getTime() : NaN;
        ...
        if (hasEnded && content.lockAfterEnd) {
          ...
          if (!content.isSeparate || (lockPrice && String(content.price || '').trim() !== lockPrice)) {
  ...
  server.post("/api/content", async (req, res) => {
    try {
      ...
      if (isLiveContent) {
        ...
        const lockPayload = normalizeLiveLockPayload(data, schedule.scheduleEnabled);
        ...
        if (schedule.scheduleEnabled) {
          ...
          const startMs = schedule.startTime ? schedule.startTime.getTime() : NaN;
          shouldNotifyScheduled = Number.isFinite(startMs) && startMs > Date.now();
        } else {
      ...
      if (isLiveContent && data.scheduleEnabled && createdContent && shouldNotifyScheduled) {
  ...
  server.get("/api/content/all", async (req, res) =>
  ...
  server.put("/api/content/:id", async (req, res) => {
    try {
      ...
      if (Array.isArray(batches) && batches.length > 0) {
        ...
        data.batch = (selectedBatches.includes("All") || selectedBatches.length > 1) ? "All" : selectedBatches[0];
      } else if (typeof data.batch === "undefined") {
        data.batch = existing.batch;
      }

      const isLiveContent = String(data.type || existing.type || '').toLowerCase() === 'live';
      let shouldNotifyScheduled = false;
      if (isLiveContent) {
        const schedule = normalizeLiveSchedulePayload(data);
        if (schedule.error) {
        ...
        const lockPayload = normalizeLiveLockPayload(data, schedule.scheduleEnabled, existing);
        if (lockPayload.error) {
          return res.status(400).json({ error: lockPayload.error });
        }
        data.lockAfterEnd = lockPayload.lockAfterEnd;
        ...
        if (schedule.scheduleEnabled) {
          const prevStartMs = existing.startTime ? new Date(existing.startTime).getTime() : NaN;
          const prevEndMs = existing.endTime ? new Date(existing.endTime).getTime() : NaN;
          const nextStartMs = schedule.startTime ? schedule.startTime.getTime() : NaN;
          const nextEndMs = schedule.endTime ? schedule.endTime.getTime() : NaN;
          const scheduleChanged = !existing.scheduleEnabled
            || prevStartMs !== nextStartMs
            || prevEndMs !== nextEndMs;

          const audienceChanged = String(existing.audienceMode || 'all') !== String(audience.audienceMode || 'all')
            || String(existing.audienceCity || '') !== String(audience.audienceCity || '');

          shouldNotifyScheduled = Number.isFinite(nextStartMs)
            && nextStartMs > Date.now()
            && (scheduleChanged || audienceChanged || typeof batches !== 'undefined');
      ...
      if (!existing.isStreamActive && data.isStreamActive) {
        ...
        const batchArr = Array.isArray(batches) && batches.length > 0 ? batches : [updatedContent.batch];
  ...
  server.get("/api/student/content", verifyToken, async (req, res) => {
    try {
      ...
      const normalizedUserBatch = String(userBatch || "").trim().toLowerCase();
      const normalizedStudentMode = String(student.classMode || "").trim().toLowerCase();
      const normalizedStudentCity = String(student.hallClass || "").trim().toLowerCase();

      const isLiveContent = (content) => {
        const typeLabel = String(content?.type || "").trim().toLowerCase();
        return typeLabel === "live" || content?.scheduleEnabled || content?.isStreamActive;
      };
      ...
      const matchesAudience = (content) => {
        ...
        if (mode === "city") {
          const targetCity = normalizeAudienceCity(content?.audienceCity).toLowerCase();
          if (!targetCity) return false;
          return normalizedStudentMode === "physical" && normalizedStudentCity === targetCity;
        }
        return true;
      };
      ...
      const visibleLessons = allLessons.filter((l) => {
        ...
        try {
          // Safely parse JSON
          batchList = typeof l.batches === 'string' ? JSON.parse(l.batches) : l.batches;
        } catch (e) {
          batchList = [];
        }
      ...
      const allContent = allContentRows.filter((c) => {
        ...
        if (!(normalizedBatch === "all" || normalizedBatch === normalizedUserBatch)) return false;
      ...
      const data = allContent.map((c) => {
        ...
        if (c.isSeparate) {
          ...
          const isParentFree = parent && (!parent.price || parent.price === "0");
  ...
  server.get("/api/student/next-exam", verifyToken, async (req, res) => {
    try {
      ...
      const normalizedStudentBatch = String(student.batch || "").trim().toLowerCase();
      const now = Date.now();

      const [mcqQuizzes, writtenQuizzes] = await Promise.all([
        McqQuiz.findAll({
          attributes: ["id", "title", "startTime", "endTime", "status", "batches", "readyTime", "timeLimit"],
      ...
      const mapCandidate = (quiz, type) => {
        const startMs = quiz?.startTime ? new Date(quiz.startTime).getTime() : NaN;
        ...
        const explicitEndMs = quiz?.endTime ? new Date(quiz.endTime).getTime() : NaN;
        const examEndMs = Number.isFinite(explicitEndMs) ? explicitEndMs : computedEndMs;
        if (!Number.isFinite(examEndMs) || now >= examEndMs) return null;

        const targetBatches = parseBatchTargets(quiz.batches)
          .map((batch) => String(batch || "").trim().toLowerCase())
          .filter(Boolean);

        const isAllBatches = targetBatches.includes("all");
        const isVisibleForStudent = isAllBatches || (normalizedStudentBatch && targetBatches.includes(normalizedStudentBatch));
      ...
      const candidates = [
        ...
        .filter(Boolean)
        .sort((a, b) => {
          if (a.phasePriority !== b.phasePriority) return a.phasePriority - b.phasePriority;
  ...
  server.get("/api/student/zoom-secrets/:contentId", verifyToken, async (req, res) => {
    try {
      const studentId = req.user.id;
      ...
      if (!content.zoomVisible) {
        return res.status(403).json({ message: "Zoom session is currently hidden." });
      }

      if (!content.zoomPasscode) {
        return res.status(500).json({ message: "Zoom passcode is missing for this class." });
      }
      ...
      if (content.isSeparate) {
        ...
        const lesson = await Lesson.findByPk(content.lessonId);
        const isParentFree = lesson && (!lesson.price || lesson.price === "0");

        const approved = await Request.findOne({
          where: { studentId, lessonId: content.lessonId, status: "approved" }
        });
      ...
      return res.status(500).json({ message: "Server error generating handshake." });
    }
  });
  ...
  server.post("/api/student/zoom-signature", verifyToken, async (req, res) => {
    try {
      ...
      const meetingNumber = String(rawMeetingNumber || "").replace(/[\s-]/g, "");

      if (!meetingNumber || !/^\d+$/.test(meetingNumber)) {
        return res.status(400).json({ message: "Valid meeting number required" });
      }
      ...
      if (!sdkKey || !sdkSecret) {
        return res.status(500).json({ message: "Zoom SDK credentials missing on server" });
      }
      ...
      return res.status(500).json({ message: "Failed to generate Zoom signature." });
    }
  });

  // 🚀 PHASE 5: SECURE ZOOM RECORDING ENDPOINT
  const normalizeShareUrl = (url = "") => String(url).trim().split("?")[0].replace(/\/$/, "").toLowerCase();
  const isZoomShareLink = (url = "") => /(^https?:\/\/)?([a-z0-9-]+\.)?zoom\.us\/rec\/share\//i.test(String(url));
  const isDirectVideoLink = (url = "") => /\.(mp4|m3u8)(\?|#|$)/i.test(String(url));
  const isRecordingDebugEnabled = () => String(process.env.RECORDING_DEBUG || "false").toLowerCase() === "true";
  const withRecordingDebug = (payload, reason, extra = {}) => (
    isRecordingDebugEnabled() ? { ...payload, debugReason: reason, ...extra } : payload
  );
  ...
  const getZoomS2SToken = async () => {
    ...
    if (!accountId || !clientId || !clientSecret) {
      throw new Error("Missing Zoom Server-to-Server OAuth env vars (ZOOM_ACCOUNT_ID, ZOOM_CLIENT_ID, ZOOM_CLIENT_SECRET)");
    }
  ...
  const resolveZoomShareToFile = async (shareUrl) => {
    ...
    const resolveFromMeetings = (meetings = []) => {
      const matchedMeeting = meetings.find((m) => normalizeShareUrl(m?.share_url) === normalized);
      if (!matchedMeeting) return null;

      const files = matchedMeeting.recording_files || [];
      const completed = files.filter((f) => f.status === "completed");
      const preferredVideo =
        completed.find((f) => String(f.file_type || "").toUpperCase() === "MP4") ||
        completed.find((f) => String(f.recording_type || "").toLowerCase().includes("shared_screen_with_speaker_view")) ||
        completed.find((f) => String(f.recording_type || "").toLowerCase().includes("speaker_view")) ||
        completed.find((f) => String(f.recording_type || "").toLowerCase().includes("gallery_view"));

      if (!preferredVideo) {
        throw new Error("No completed MP4/video recording file found for this Zoom share link");
      }

      const fileUrl = preferredVideo.download_url || preferredVideo.play_url || preferredVideo.file_url;
      if (!fileUrl) throw new Error("No playable recording URL in Zoom API response");
    ...
    // Attempt 1: single-user scope (fast path)
    {
      ...
      for (let page = 0; page < 10; page++) {
        const recRes = await axios.get("https://api.zoom.us/v2/users/me/recordings", {
        ...
        if (resolved) {
          zoomRecordingCache.set(normalized, { data: resolved, expiresAt: Date.now() + 10 * 60 * 1000 });
          return resolved;
        }
    ...
    // Attempt 2: account-wide scan (recording may belong to another Zoom user in same account)
    {
      ...
      for (let userPage = 0; userPage < 10; userPage++) {
        const usersRes = await axios.get("https://api.zoom.us/v2/users", {
          headers: { Authorization: `Bearer ${token}` },
          params: { page_size: 100, next_page_token: usersPageToken, status: "active" }
        });
        ...
        for (const u of users) {
          ...
          for (let recPage = 0; recPage < 10; recPage++) {
            const recRes = await axios.get(`https://api.zoom.us/v2/users/${encodeURIComponent(u.id)}/recordings`, {
              headers: { Authorization: `Bearer ${token}` },
    ...
    throw new Error("Zoom share link could not be resolved via Zoom API. Ensure it belongs to this Zoom account.");
  };

  server.get("/api/student/zoom-recording/:contentId", verifyToken, async (req, res) => {
    try {
      const studentId = req.user.id;
      const contentId = req.params.contentId;
      ...
      if (content.isSeparate) {
        if (isFreeVideo) {
          ...
          const approved = await Request.findOne({ where: { studentId, contentId, status: "approved" } });
          hasAccess = !!approved;
        }
      } else {
        const lesson = await Lesson.findByPk(content.lessonId);
        const isParentFree = lesson && (!lesson.price || lesson.price === "0");
        const approved = await Request.findOne({ where: { studentId, lessonId: content.lessonId, status: "approved" } });
        hasAccess = isParentFree || !!approved;
      }
      ...
      if (!hasAccess) {
        ...
        return res.status(402).json(withRecordingDebug({
          success: false,
          message: "You must purchase this content to access the recording"
        }, "payment_or_access_denied", { contentId, studentId }));
      }
      ...
      if (!direct && !zoomShare) {
        return res.status(422).json(withRecordingDebug({
          success: false,
          message: "Recording link must be direct .mp4/.m3u8 or Zoom share link."
        }, "unsupported_recording_link_format", { contentId, recordingLink: recordingUrl }));
      }
      ...
      res.status(500).json(withRecordingDebug({
        success: false,
        message: "Server error accessing recording"
      }, "recording_access_exception", { details: e?.message || String(e) }));
    }
  });

  server.get("/api/student/zoom-recording-stream/:contentId", async (req, res) => {
    try {
      const contentId = req.params.contentId;
      ...
      if (streamToken) {
        try {
          const decoded = jwt.verify(streamToken, process.env.JWT_SECRET || "secret");
          if (decoded?.purpose !== "secure_recording_stream") {
            return res.status(401).json(withRecordingDebug({ success: false, message: "Invalid stream token" }, "stream_invalid_purpose", { contentId }));
          }
          if (String(decoded?.contentId) !== String(contentId)) {
            return res.status(401).json(withRecordingDebug({ success: false, message: "Stream token/content mismatch" }, "stream_token_content_mismatch", { contentId }));
          }
          studentId = decoded.id;
        } catch (err) {
          return res.status(401).json(withRecordingDebug({ success: false, message: "Stream token expired or invalid" }, "stream_token_invalid", { contentId }));
        }
      }
      ...
      if (!studentId) {
        ...
        if (!cookieToken) {
          return res.status(403).json(withRecordingDebug({ success: false, message: "No Token" }, "stream_missing_auth", { contentId }));
        }
        try {
          const decoded = jwt.verify(cookieToken, process.env.JWT_SECRET || "secret");
          studentId = decoded?.id;
        } catch (err) {
          return res.status(401).json(withRecordingDebug({ success: false, message: "Unauthorized" }, "stream_cookie_invalid", { contentId }));
        }
      }

      const content = await Content.findByPk(contentId);
      if (!content) {
        return res.status(404).json(withRecordingDebug({ success: false, message: "Recording not found or has been deleted" }, "stream_content_not_found", { contentId }));
      }
      if (!content.recordingVisible) {
        return res.status(404).json(withRecordingDebug({ success: false, message: "Recording is hidden by admin" }, "stream_recording_hidden_by_admin", { contentId }));
      }
      if (!content.recordingLink) {
        return res.status(404).json(withRecordingDebug({ success: false, message: "Recording link is missing" }, "stream_recording_link_missing", { contentId }));
      }
      ...
      if (!hasAccess) {
        return res.status(402).json(withRecordingDebug({ success: false, message: "You must purchase this content to access the recording" }, "stream_payment_or_access_denied", { contentId, studentId }));
      }
      ...
      const zoomShare = isZoomShareLink(recordingUrl);
      if (!direct && !zoomShare) {
        return res.status(422).json(withRecordingDebug({ success: false, message: "Unsupported recording link format" }, "stream_unsupported_recording_link_format", { contentId, recordingLink: recordingUrl }));
      }
      ...
      upstreamHeaders["User-Agent"] = req.headers["user-agent"] || "Mozilla/5.0";
      ...
      if (!res.headersSent) {
        res.status(500).json(withRecordingDebug({ success: false, message: "Failed to stream recording" }, "stream_exception", { details: e?.message || String(e) }));
      }
    }
  });
  ...
  server.post("/api/login", async (req, res) => {
    try {
      ...
      if (!isPasswordCorrect) {
        ...
        return res.status(401).json({ message: "Invalid Mobile Number / Password." });
      }

      if (user.role === "student" && onlineUsers.has(String(user.id))) {
        const existingSession = onlineUsers.get(String(user.id));
        const socketServer = typeof io !== 'undefined' ? io : (global.io || req.app.get('io'));

        if (socketServer && existingSession?.socketId) {
          socketServer.to(existingSession.socketId).emit('concurrent_login', {
            message: 'Security Alert: Your account was logged in from another device. This session has been closed.'
          });
        }
    ...
    try {
      ...
      if (classMode === "physical") {
        ...
        if (!cities.includes(hallClass)) {
          return res.status(400).json({ message: "Selected Hall Class city is invalid" });
        }
      }
      ...
      res.status(500).json({ message: e?.message || "Failed to update class mode" });
    }
  });
  ...
  server.post("/api/register", async (req, res) => {
    try {
      const { password, name, mobile, address, nic: nicInput, batch, classMode: modeInput, hallClass: hallInput } = req.body;
      const classMode = normalizeClassMode(modeInput);
      const hallClass = normalizeHallClass(hallInput);
      const nic = String(nicInput || "").trim().toUpperCase();
      ...
      sendNewAccountCreatedAdminAlert({ student: user }).catch((error) => {
        console.error('[ADMIN NOTIFY] New account created alert error:', error?.message || error);
      });
  ...
  server.post("/api/me/email", verifyToken, async (req, res) => {
    try {
      ...
      return res.status(500).json({ message: e?.message || "Failed to update email" });
    }
  });

  server.post("/api/me/nic", verifyToken, async (req, res) => {
    try {
      const nic = String(req.body?.nic || "").trim().toUpperCase();
      if (!isValidSriLankanNIC(nic)) {
      ...
      if (String(user.nic || "").trim()) {
        return res.status(400).json({ message: "NIC is already saved and cannot be edited" });
      }
      ...
      return res.status(500).json({ message: e?.message || "Failed to update NIC" });
    }
  });

  server.post("/api/admin/reset-student-email-nic", requireAdmin, async (req, res) => {
    try {
      ...
      console.error('[ADMIN RESET] Failed to clear student email/NIC:', e?.message || e);
      return res.status(500).json({ message: 'Failed to reset student email/NIC' });
    }
  });
  ...
  server.post("/api/admin/request-action", async (req, res) => {
    try {
      ...
      const normalizedStatus = incomingStatus === 'rejected' ? 'declined' : incomingStatus;
      ...
      if (['approved', 'declined'].includes(normalizedStatus) && previousStatus !== normalizedStatus) {
        ...
        sendRequestDecisionAlertToStudent({
          ...
          console.error('[REQUEST ALERT] Student decision notify error:', error?.message || error);
        });
      }
  ...
  server.post("/api/student/send-welcome-notification", verifyToken, async (req, res) => {
    ...
    try {
      ...
      if (!student.welcomeNotificationEligible) {
        return res.json({ success: false, skipped: true, reason: 'not_eligible_existing_account' });
      }

      if (student.welcomeNotificationSent) {
        return res.json({ success: false, skipped: true, reason: 'already_sent' });
      }

      console.log(`[WELCOME NOTIFY] Sending welcome notification for student ${student.id}...`);
      ...
      return res.status(500).json({ error: 'Failed to send welcome notification' });
    }
  });
  ...
  server.post(
    ...
    async (req, res) => {
      try {
        ...
        const notificationMsg = (type === "PDF_ACCESS" || type === "PASTPAPER_ACCESS")
          ? `Document Request: ${student.name}`
          : `Payment: ${student.name}`;

        io.to("admin_room").emit("new_request_received", { message: notificationMsg });
        ...
        sendRequestSubmissionAlertToAdmins({
          ...
          console.error('[REQUEST ALERT] Admin submission notify error:', error?.message || error);
        });
  ...
  server.get("/api/settings/notifications", async (req, res) => {
    const emailSetting = await Setting.findOne({ where: { key: "notifications_email" } });
    const pushSetting = await Setting.findOne({ where: { key: "notifications_push" } });
    res.json({
  ...
  server.post("/api/settings/notifications", async (req, res) => {
    ...
    io.emit("notifications_settings_update", { emailEnabled: nextEmail, pushEnabled: nextPush });
  ...
  server.get("/api/admin/student/:id", verifyToken, requireAdmin, async (req, res) => {
    try {
  ...
  server.post("/api/admin/students/toggle-status", async (req, res) => {
    try {
      ...
      if (newStatus === "deactivated") {
        const socketServer = typeof io !== 'undefined' ? io : (global.io || req.app.get('io'));
        const affectedSessions = Array.from(onlineUsers.entries()).filter(([, info]) => String(info.userId) === String(id));
        ...
        sendAccountStatusAlertToStudent({
          ...
          console.error('[ACCOUNT ALERT] Suspend notify error:', error?.message || error);
        });
      } else {
        sendAccountStatusAlertToStudent({
          user,
          ...
          console.error('[ACCOUNT ALERT] Reactivate notify error:', error?.message || error);
        });
      }
  ...
  server.post("/api/admin/unblock", async (req, res) => {
    ...
    if (user) {
      ...
      sendAccountStatusAlertToStudent({
        ...
        console.error('[ACCOUNT ALERT] Unblock notify error:', error?.message || error);
      });
    ...
    try {
      ...
      if (user) {
        ...
        sendAccountStatusAlertToStudent({
          ...
          console.error('[ACCOUNT ALERT] Security-ban notify error:', error?.message || error);
        });
  ...
  server.get("/api/admin/backup", verifyToken, requireAdmin, async (req, res) => {
    try {
      const data = {};
      ...
      const backup = {
        ...
        storageManifest: {
          ...
          publicUploads: buildFolderManifest(path.join(__dirname, "public", "uploads")),
        },
      };
      ...
      return res.status(500).json({ message: "Backup failed", error: e.message });
    }
  });

  server.post("/api/admin/restore", verifyToken, requireAdmin, async (req, res) => {
    const payload = req.body;
    if (!payload || !payload.data || typeof payload.data !== "object") {
      return res.status(400).json({ message: "Invalid backup payload" });
    }

    const unknownModels = Object.keys(payload.data).filter((name) => !BACKUP_MODELS[name]);
    ...
    try {
      await sequelize.query("SET FOREIGN_KEY_CHECKS = 0", { transaction: trx });

      for (const modelName of [...RESTORE_MODEL_ORDER].reverse()) {
        const model = BACKUP_MODELS[modelName];
        await model.destroy({ where: {}, force: true, truncate: true, transaction: trx });
      }

      for (const modelName of RESTORE_MODEL_ORDER) {
        const rows = Array.isArray(payload.data[modelName]) ? payload.data[modelName] : [];
        if (rows.length > 0) {
      ...
      await sequelize.query("SET FOREIGN_KEY_CHECKS = 1", { transaction: trx });
      await trx.commit();
      ...
      await sequelize.query("SET FOREIGN_KEY_CHECKS = 1", { transaction: trx }).catch(() => { });
      await trx.rollback();
      console.error("Restore failed:", e);
      return res.status(500).json({ message: "Restore failed", error: e.message });
    }
  });

  server.post("/api/admin/restart", verifyToken, requireAdmin, async (req, res) => {
    try {
      ...
      res.status(202).json({
        success: true,
        message: "Restart queued. The website may disconnect for a few seconds.",
      });
      ...
      return res.status(500).json({ message: "Restart failed", error: e.message });
    }
  });
  ...
  server.get("/api/pastpapers/:id/pdf-token", verifyToken, async (req, res) => {
    ...
    try {
      ...
      if (!file || !file.isVisible) return res.status(404).json({ message: "PDF unavailable." });
      ...
      if (file.isFree) {
        const pdfToken = jwt.sign(
          { id: studentId, contentId: file.id, purpose: "secure_pdf_view", isPastPaper: true },
          process.env.JWT_SECRET || "secret",
          { expiresIn: '24h' }
        );
        return res.json({ success: true, token: pdfToken, expiresInMins: timeLimitMins, isFree: true, pageImages: content.pageImages });
      }

      // 🔒 PAID MODE: Check request
      const approvedRequest = await Request.findOne({
        where: { studentId, contentId: fileId, type: "PASTPAPER_ACCESS", status: "approved" },
        order: [['createdAt', 'DESC']]
      });

      if (!approvedRequest) return res.status(403).json({ message: "Access denied." });
      ...
      const elapsedMins = (new Date().getTime() - new Date(startTime).getTime()) / (1000 * 60);
      const remainingMins = timeLimitMins - elapsedMins;
      ...
      return res.json({ success: true, token: pdfToken, expiresInMins: remainingMins, isFree: false, pageImages: content.pageImages });
  ...
  // STATUS CHECKER
  server.get("/api/student/pastpaper-status/:id", verifyToken, async (req, res) => {
    try {
      const file = await PastPaperFile.findByPk(req.params.id);
      if (file && file.isFree) return res.json({ status: "approved" });

      const request = await Request.findOne({
        where: { studentId: req.user.id, contentId: req.params.id, type: "PASTPAPER_ACCESS" },
        order: [['createdAt', 'DESC']]
      });
      ...
      if (request.status === 'approved' && request.accessedAt && file) {
        const elapsedMins = (new Date().getTime() - new Date(request.accessedAt).getTime()) / (1000 * 60);
  ...
  server.get("/api/pastpapers/files", verifyToken, async (req, res) => {
    try {
      ...
      const data = files.map(f => {
        ...
        fileData.isUnlocked = fileData.isFree || approvedIds.includes(fileData.id);
        return fileData;
      });
  ...
  server.post("/api/admin/pastpapers/folders", verifyToken, async (req, res) => {
    try {
      ...
      await PastPaperFolder.create({
        name,
        batches: JSON.stringify(batches && batches.length > 0 ? batches : ["All"]),
  ...
  server.post("/api/admin/pastpapers/files", verifyToken, async (req, res) => {
    try {
      ...
      if (!filename) return res.status(400).json({ message: "No file provided" });

      const newFile = await PastPaperFile.create({
        title, folderId, pdfFile: filename, isFree: isFree === true, timeLimit: timeLimit || 60
      });
  ...
  server.delete("/api/admin/pastpapers/folders/:id", verifyToken, async (req, res) => {
    try {
      if (req.user.role !== 'admin') return res.status(403).send("Forbidden");
      ...
      const getAllSubFolderIds = async (parentId) => {
        let ids = [];
        const subfolders = await PastPaperFolder.findAll({ where: { parentId } });
      ...
      const allFolderIdsToDelete = [targetFolderId, ...(await getAllSubFolderIds(targetFolderId))];

      await PastPaperFile.destroy({ where: { folderId: { [Op.in]: allFolderIdsToDelete } } });
      await PastPaperFolder.destroy({ where: { id: { [Op.in]: allFolderIdsToDelete } } });
  ...
  server.delete("/api/admin/pastpapers/files/:id", verifyToken, async (req, res) => {
    try {
      if (req.user.role !== 'admin') return res.status(403).send("Forbidden");
  ...
  // 7. RENAME FOLDER
  server.put("/api/admin/pastpapers/folders/:id", verifyToken, async (req, res) => {
    try {
      if (req.user.role !== 'admin') return res.status(403).send("Forbidden");
      const newFile = await PastPaperFile.create({
        title, folderId, pdfFile: filename, isFree: isFree === true, timeLimit: timeLimit || 60
  ...
  // 8. FULL EDIT PDF
  server.put("/api/admin/pastpapers/files/:id", verifyToken, async (req, res) => {
    try {
      if (req.user.role !== 'admin') return res.status(403).send("Forbidden");
  ...
  server.post("/api/student/expire-pdf/:contentId", verifyToken, async (req, res) => {
    try {
  ...
  server.post("/api/admin/vault/upload", verifyToken, vaultUpload.single("file"), async (req, res) => {
    try {
      if (req.user.role !== "admin") return res.status(403).json({ message: "Forbidden" });
  ...
  server.get("/api/student/written/view/:id", verifyToken, async (req, res) => {
    try {
      ...
      if (previousResult) {
        return res.json({
          id: quiz.id, title: quiz.title, totalMarks: quiz.totalMarks,
          previousResult: {
      ...
      res.json({
        id: quiz.id, title: quiz.title, pdfFile: quiz.pdfFile,
        readyTime: quiz.readyTime, timeLimit: quiz.timeLimit, uploadGraceTime: quiz.uploadGraceTime,
        totalMarks: quiz.totalMarks, startTime: quiz.startTime, status: quiz.status
      });
    } catch (e) { res.status(500).json({ error: e.message }); }
  });

  // 2. THE DROPZONE RECEIVER (Accepts up to 10 files)
  server.post("/api/student/written/submit", verifyToken, uploadWritten.array('answers', 30), async (req, res) => {
    try {
      const studentId = req.user.id;
      const { quizId } = req.body;
      const files = req.files;

      if (!files || files.length === 0) return res.status(400).json({ message: "No files detected!" });

      const fileUrls = files.map(f => `/uploads/written-answers/${f.filename}`);

      // 🚀 THE FIX: Check if they ignited the engine first
      const existing = await WrittenResult.findOne({ where: { studentId, quizId } });
      ...
      res.json({ success: true, message: "Answer script secured in the Vault!" });
    } catch (e) { res.status(500).json({ error: e.message }); }
  });
  ...
  server.delete("/api/admin/vault/files/:filename", verifyToken, async (req, res) => {
    try {
      if (req.user.role !== "admin") return res.status(403).json({ message: "Forbidden" });
      ...
      await Content.update({ pdfFile: null }, { where: { pdfFile: filename } });
  ...
  server.put("/api/admin/vault/files/rename", verifyToken, async (req, res) => {
    try {
      ...
      if (!oldFilename || !newFilename) return res.status(400).json({ message: "Missing filenames" });
      ...
      let safeNewName = newFilename.trim().replace(/[^a-zA-Z0-9\s-]/g, '').replace(/\s+/g, '_');
      ...
      await Content.update({ pdfFile: safeNewName }, { where: { pdfFile: oldFilename } });
      await PastPaperFile.update({ pdfFile: safeNewName }, { where: { pdfFile: oldFilename } });
  ...
  const serveSecureVaultFile = async (req, res) => {
    try {
      const rawInput = String(req.params.filename || req.params[0] || "").trim();
      ...
      const normalizedInput = decodedInput.replace(/\\/g, "/").replace(/^\/+/, "");
      console.log("[SECURE-VAULT] normalizedInput:", normalizedInput);

      const filenameVariants = Array.from(new Set([
        normalizedInput,
      ...
      for (const candidateName of filenameVariants) {
        ...
        console.log(`[SECURE-VAULT] Checking candidate: ${candidateName} => ${candidatePath}`);

        // Security: Prevent Directory Traversal attacks (e.g. ../../etc/passwd)
        if (candidatePath !== baseVaultDir && !candidatePath.startsWith(baseVaultDir + path.sep)) {
      ...
      try {
        ...
        console.warn("[SECURE-VAULT] Could not set Content-Length/Accept-Ranges", e);
      }
  ...
  function getDiskUsage() {
    return new Promise((resolve) => {
      if (process.platform === 'win32') return resolve({ percent: 0, used: '0GB', total: '100GB' });
      exec('df -h /', (err, stdout) => {
        if (err) return resolve({ percent: 0, used: '0', total: '0' });
        const lines = stdout.trim().split('\n');
        const diskLine = lines[lines.length - 1].replace(/\s+/g, ' ').split(' ');
        resolve({
          total: diskLine[1],
          used: diskLine[2],
  ...
  fetch('https://api.ipify.org?format=json').then(r => r.json()).then(d => publicIp = d.ip).catch(() => { });
  ...
  setInterval(async () => {
    ...
    let recentLogs = [];
    try {
      const logData = fs.readFileSync('/root/.pm2/logs/sftbeta-out.log', 'utf8');
      const lines = logData.trim().split('\n');
      recentLogs = lines.slice(-20);
    } catch (e) { recentLogs = ["Waiting for logs..."]; }
  ...
  server.get("/api/mcq/quizzes", verifyToken, async (req, res) => {
    try {
      ...
      if (req.user.role === 'student') {
        ...
        const results = await McqResult.findAll({ where: { studentId: req.user.id } });
  ...
  server.post("/api/admin/mcq/quizzes", verifyToken, async (req, res) => {
    try {
      if (req.user.role !== 'admin') return res.status(403).send("Forbidden");
      const { title, folderId, pdfFile, answerKey, timeLimit, readyTime, totalQuestions, batches, startTime, endTime, status } = req.body;
      if (!pdfFile || !answerKey) return res.status(400).json({ message: "PDF and Answer Key required" });

      const createdQuiz = await McqQuiz.create({
        title, folderId, pdfFile,
        answerKey: JSON.stringify(answerKey),
        timeLimit: timeLimit || 60, readyTime: readyTime || 0, totalQuestions: totalQuestions || 10,
        batches: batches ? JSON.stringify(batches) : '["All"]',
        startTime: startTime || null, endTime: endTime || null, status: status || 'live'
      });

      if (createdQuiz.status === 'scheduled') {
        sendMcqScheduledNotifications({ quiz: createdQuiz, trigger: 'created' }).catch((error) => {
          console.error('[MCQ NOTIFY] Create schedule notification error:', error?.message || error);
        });
      }
  ...
  server.delete("/api/admin/mcq/folders/:id", verifyToken, async (req, res) => {
    try {
      ...
      const quizzesToDelete = await McqQuiz.findAll({ where: { folderId: targetFolderId } });
  ...
  const getMcqTiming = ({ quiz, attempt, nowMs = Date.now(), fallbackStartMs = null }) => {
    const scheduledStartMs = quiz?.startTime ? new Date(quiz.startTime).getTime() : NaN;
    const attemptStartMs = attempt?.createdAt ? new Date(attempt.createdAt).getTime() : NaN;
    ...
    return { nowMs, startMs, readyEndMs, examEndMs, readyMs, timeLimitMs, source };
  };
  ...
  server.post("/api/student/mcq/submit", verifyToken, async (req, res) => {
    try {
      ...
      const graceMs = 10000;
      if (Number.isFinite(timing.examEndMs) && nowMs > timing.examEndMs + graceMs) {
      ...
      const totalQuestions = quiz.totalQuestions || Object.keys(correctAnswers).length;
      let score = 0;
      ...
      const submittedMs = result?.createdAt ? new Date(result.createdAt).getTime() : nowMs;
      const timeUsedSeconds = getMcqTimeUsedSeconds(submittedMs, timing);
      const startedAtIso = Number.isFinite(timing.startMs) ? new Date(timing.startMs).toISOString() : null;
      const submittedAtIso = result?.createdAt ? new Date(result.createdAt).toISOString() : null;
  ...
  server.get("/api/student/mcq/view/:id", verifyToken, async (req, res) => {
    try {
      ...
      const fallbackStartMs = previousResult?.createdAt ? new Date(previousResult.createdAt).getTime() : null;
      const timing = getMcqTiming({ quiz, attempt, nowMs, fallbackStartMs });
      const isClosedForReview = quiz.status === 'ended' || (quiz.status === 'scheduled' && nowMs >= timing.examEndMs);
      const timingPayload = {
      ...
      if (previousResult) {
        ...
        try { parsedStudentAnswers = JSON.parse(previousResult.studentAnswers || '{}'); } catch (e) { }

        const startedAtIso = Number.isFinite(timing.startMs) ? new Date(timing.startMs).toISOString() : null;
        const submittedAtIso = previousResult.createdAt ? new Date(previousResult.createdAt).toISOString() : null;
        const submittedMs = submittedAtIso ? new Date(submittedAtIso).getTime() : NaN;
        const timeUsedSeconds = getMcqTimeUsedSeconds(submittedMs, timing);

        return res.json({
          id: quiz.id, title: quiz.title, pdfFile: quiz.pdfFile,
          timeLimit: quiz.timeLimit, readyTime: quiz.readyTime,
      ...
      res.json({
        ...
        startTime: quiz.startTime, status: quiz.status,
        timing: timingPayload
      });
    } catch (e) {
      res.status(500).json({ error: e.message });
  ...
  server.get("/api/admin/mcq/results/:quizId", verifyToken, async (req, res) => {
    try {
      if (req.user.role !== 'admin') return res.status(403).send("Forbidden");
      ...
      for (const a of attempts) {
        ...
        const startedAt = a.createdAt ? new Date(a.createdAt).toISOString() : null;
      ...
      const payload = results.map((r) => {
        ...
        const submittedMs = row?.createdAt ? new Date(row.createdAt).getTime() : NaN;
        const attemptStartedAt = attemptStartMap.get(String(row.studentId)) || null;
        ...
        return {
          ...row,
          attemptStartedAt: Number.isFinite(timing.startMs) ? new Date(timing.startMs).toISOString() : null,
          writeStartedAt: Number.isFinite(timing.readyEndMs) ? new Date(timing.readyEndMs).toISOString() : null,
          timeUsedSeconds
        };
      });
  ...
  server.get("/api/admin/mcq/results-history/:quizId", verifyToken, async (req, res) => {
    try {
      if (req.user.role !== 'admin') return res.status(403).send("Forbidden");
      ...
      const history = await McqResultHistory.findAll({
        where: { quizId: req.params.quizId },
        include: [{ model: User, attributes: ['name', 'mobile'] }],
        order: [['archivedAt', 'DESC'], ['score', 'DESC']]
      });
  ...
  server.put("/api/admin/mcq/quizzes/:id", verifyToken, async (req, res) => {
    try {
      ...
      const prevStartMs = quiz.startTime ? new Date(quiz.startTime).getTime() : null;
      const nextStatus = req.body.status !== undefined ? req.body.status : quiz.status;
      const nextStartMs = req.body.startTime !== undefined
        ? (req.body.startTime ? new Date(req.body.startTime).getTime() : null)
        : prevStartMs;
      ...
      const shouldNotifyScheduledStudents =
        nextStatus === 'scheduled' &&
        (hasStartChanged || prevStatus !== 'scheduled' || req.body.batches !== undefined);
      ...
      if (req.body.totalQuestions !== undefined) quiz.totalQuestions = req.body.totalQuestions;
      ...
      if (req.body.answerKey) {
        quiz.answerKey = typeof req.body.answerKey === 'string'
          ? req.body.answerKey
      ...
      if (shouldResetForReschedule) {
        ...
        const resultsToArchive = await McqResult.findAll({
          where: { quizId: quiz.id },
          attributes: ['id', 'studentId', 'quizId', 'score', 'totalQuestions', 'studentAnswers', 'createdAt']
        });
        ...
        if (resultsToArchive.length > 0) {
          await McqResultHistory.bulkCreate(
            resultsToArchive.map((r) => ({
              ...
              attemptStartedAt: attemptStartMap.get(String(r.studentId)) || null,
      ...
      if (shouldNotifyScheduledStudents) {
        sendMcqScheduledNotifications({ quiz, trigger: hasStartChanged ? 'rescheduled' : 'updated' }).catch((error) => {
          console.error('[MCQ NOTIFY] Update schedule notification error:', error?.message || error);
        });
      }
  ...
  server.post("/api/student/ai-chat", verifyToken, async (req, res) => {
    try {
      ...
      for (const msg of rawHistory) {
        ...
        if (text.includes("Hello! I am") || text.includes("API ERROR") || text.includes("කරුණාකර") || text.includes("SYSTEM ERROR")) continue;
      ...
      const politeErrorMsg = "👑 I am currently helping a massive number of students! My systems are resting for a few seconds. කරුණාකර මොහොතකින් නැවත උත්සාහ කරන්න. (Please try again in a moment!)";
      res.status(500).json({ error: politeErrorMsg });
    }
  });

  // 8. GET ALL SUBMISSIONS FOR A SPECIFIC EXAM (CEO GRADING DESK)
  server.get("/api/admin/written/results/:quizId", verifyToken, async (req, res) => {
    try {
      if (req.user.role !== 'admin') return res.status(403).send("Forbidden");
  ...
  server.put("/api/admin/written/results/:resultId", verifyToken, async (req, res) => {
    try {
      if (req.user.role !== 'admin') return res.status(403).send("Forbidden");

      // 1. Catch the new Base64 image data from the frontend
      const { score, feedback, gradingStatus, gradedImageBase64, gradedImageIndex } = req.body;

      // 2. Fetch the exact student submission from the database
      const submission = await WrittenResult.findByPk(req.params.resultId);
      if (!submission) return res.status(404).json({ message: "Submission not found in the Matrix." });
      ...
      if (gradedImageBase64) {
        ...
        const base64Data = gradedImageBase64.replace(/^data:image\/\w+;base64,/, "");
        ...
        const filePath = path.join(__dirname, 'uploads', 'written-answers', newFileName);
        fs.writeFileSync(filePath, buffer);

        // 🤯 THE MAGIC TRICK: 
        // Replace the original blank photo with the newly GRADED photo URL in the array!
        currentFiles[gradedImageIndex] = `/uploads/written-answers/${newFileName}`;
      }
  ...
  server.put("/api/admin/written/quizzes/:quizId/publish", verifyToken, async (req, res) => {
    try {
      if (req.user.role !== 'admin') return res.status(403).send("Forbidden");
  ...
  server.get("/api/admin/written/all-quizzes", verifyToken, async (req, res) => {
    try {
      if (req.user.role !== 'admin') return res.status(403).send("Forbidden");
      const quizzes = await WrittenQuiz.findAll({ order: [['createdAt', 'DESC']] });
      res.json(quizzes);
    } catch (e) { res.status(500).json({ error: e.message }); }
  });
  // =========================================================================
  ...
  server.put("/api/admin/written/folders/:id", verifyToken, async (req, res) => {
    try {
      if (req.user.role !== 'admin') return res.status(403).send("Forbidden");
      await WrittenFolder.update({ name: req.body.name }, { where: { id: req.params.id } });
      res.json({ success: true });
    } catch (e) { res.status(500).json({ error: e.message }); }
  });
  ...
  server.get("/api/written/quizzes", verifyToken, async (req, res) => {
    try {
      ...
      if (req.user.role === 'student') {
        ...
        const results = await WrittenResult.findAll({ where: { studentId: req.user.id } });
  ...
  server.put("/api/admin/written/quizzes/:id", verifyToken, async (req, res) => {
    try {
      if (req.user.role !== 'admin') return res.status(403).send("Forbidden");
      const quiz = await WrittenQuiz.findByPk(req.params.id);
      if (!quiz) return res.status(404).send("Not found");
  ...
  server.delete("/api/admin/written/folders/:id", verifyToken, async (req, res) => {
    try {
      if (req.user.role !== 'admin') return res.status(403).send("Forbidden");
      const targetFolderId = parseInt(req.params.id);

      const quizzesToDelete = await WrittenQuiz.findAll({ where: { folderId: targetFolderId } });
      const quizIds = quizzesToDelete.map(q => q.id);

      if (quizIds.length > 0) {
        // 🚀 PHYSICAL WIPE: Delete all uploaded images from the server to save space!
        const results = await WrittenResult.findAll({ where: { quizId: { [Op.in]: quizIds } } });
        for (const r of results) {
          let fileUrls = [];
          try { fileUrls = JSON.parse(r.fileUrls || r.answers || '[]'); } catch (e) { }
        ...
        // Nuke student leaderboard results
        await WrittenResult.destroy({ where: { quizId: { [Op.in]: quizIds } } });
  ...
  server.delete("/api/admin/written/quizzes/:id", verifyToken, async (req, res) => {
    try {
      if (req.user.role !== 'admin') return res.status(403).send("Forbidden");
  ...
  server.post("/api/admin/written/quizzes", verifyToken, async (req, res) => {
    try {
      if (req.user.role !== 'admin') return res.status(403).send("Forbidden");
      const { title, folderId, pdfFile, readyTime, timeLimit, uploadGraceTime, totalMarks, batches, startTime, endTime, status } = req.body;
      if (!pdfFile) return res.status(400).json({ message: "PDF Vault File is required" });

      await WrittenQuiz.create({
        title, folderId: folderId || null, pdfFile,
        readyTime: readyTime || 5, timeLimit: timeLimit || 120, uploadGraceTime: uploadGraceTime || 10, totalMarks: totalMarks || 100,
        batches: batches ? JSON.stringify(batches) : '["All"]',
        startTime: startTime || null, endTime: endTime || null, status: status || 'scheduled'
      });
  ...
  server.post("/api/student/written/submit", verifyToken, upload.array('answers', 10), async (req, res) => {
    try {
      const { quizId } = req.body;
      const userId = req.user.id;

      // 1. Safety Checks
      if (!quizId) return res.status(400).json({ message: "Missing Exam ID in Matrix." });
      if (!req.files || req.files.length === 0) {
        return res.status(400).json({ message: "No answer scripts were detected!" });
      }
      ...
      const existingSubmission = await WrittenResult.findOne({ where: { userId, quizId } });
      if (existingSubmission) {
        return res.status(400).json({ message: "Vault Violation: You have already submitted this exam." });
      }
  ...
  server.get("/api/written-answers/:filename", (req, res) => {
    const filePath = path.join(__dirname, 'uploads', 'written-answers', req.params.filename);
    if (fs.existsSync(filePath)) {
      res.sendFile(filePath);
    } else {
      res.status(404).send("File not found in the Matrix.");
  ...
  server.delete("/api/student/written/nuke-images/:quizId", verifyToken, async (req, res) => {
    try {
      const result = await WrittenResult.findOne({
        where: { quizId: req.params.quizId, studentId: req.user.id }
      });

      if (!result || result.fileUrls === '[]') return res.json({ success: true });

      let fileUrls = [];
      try { fileUrls = JSON.parse(result.fileUrls); } catch (e) { }
  ...
  server.get("/api/student/leaderboards/quizzes/:type", verifyToken, async (req, res) => {
    try {
      ...
      if (type === 'mcq') {
        ...
        quizzes = await McqQuiz.findAll({
          attributes: ['id', 'title', 'totalQuestions', 'batches', 'startTime', 'endTime', 'status', 'timeLimit', 'readyTime'],
          order: [['createdAt', 'DESC']]
        });
  ...
  server.get("/api/student/leaderboards/results/:type/:quizId", verifyToken, async (req, res) => {
    try {
      ...
      if (type === 'mcq') {
        const quiz = await McqQuiz.findByPk(quizId, {
          attributes: ['id', 'title', 'startTime', 'endTime', 'status', 'timeLimit', 'readyTime', 'totalQuestions']
        });
        if (!quiz) {
        ...
        const now = new Date();
        const isLocked = Boolean(unlockAt && now < unlockAt && quiz.status !== 'ended');
        ...
        results = await McqResult.findAll({
          where: { quizId },
          include: [{ model: User, attributes: ['id', 'name', 'batch', 'avatar'] }],
      ...
      for (const r of results) {
        ...
        const rowReactions = allReactions.filter(reaction => reaction.entryId === r.id);
        const reactionCounts = {};
        let myReaction = null;

        rowReactions.forEach(reaction => {
          reactionCounts[reaction.reactionId] = (reactionCounts[reaction.reactionId] || 0) + 1;
      ...
      if (rankedLeaderboard.length > TOP_LIMIT && cutoffScore !== null) {
        publicRows = rankedLeaderboard.filter((entry, idx) => idx < TOP_LIMIT || Number(entry.score) === cutoffScore);
      }

      const selfRow = rankedLeaderboard.find((entry) => String(entry.studentId) === String(studentId)) || null;
      const selfIsPublic =
        selfRow !== null &&
        publicRows.some((entry) => String(entry.studentId) === String(selfRow.studentId));
  ...
  server.post("/api/student/leaderboards/react", verifyToken, async (req, res) => {
    try {
      const studentId = req.user.id;
      ...
      const existing = await Reaction.findOne({
        where: { userId: studentId, entryId: attemptId, engineType: engineType }
      });
  ...
  server.get("/api/admin/ghost-buster", verifyToken, async (req, res) => {
    try {
      ...
      let trappedMcq = 0, trappedWritten = 0, orphanedMcqRes = 0, orphanedWrittenRes = 0;
      ...
      const allWrittenFolders = await WrittenFolder.findAll({ attributes: ['id'] });
      ...
      for (const q of allWrittenQuizzes) {
        if (q.folderId !== null && !validWrittenFIds.includes(q.folderId)) {
          // Physical Image Wipe
          const results = await WrittenResult.findAll({ where: { quizId: q.id } });
          for (const r of results) {
            let fileUrls = [];
            try { fileUrls = JSON.parse(r.fileUrls || r.answers || '[]'); } catch (e) { }
            for (const url of fileUrls) {
      ...
      const validMcqQIds = (await McqQuiz.findAll({ attributes: ['id'] })).map(q => q.id);
      if (validMcqQIds.length > 0) orphanedMcqRes = await McqResult.destroy({ where: { quizId: { [Op.notIn]: validMcqQIds } } });
      else orphanedMcqRes = await McqResult.destroy({ where: {} });

      const validWrittenQIds = (await WrittenQuiz.findAll({ attributes: ['id'] })).map(q => q.id);
      let orphanWritten = [];
      if (validWrittenQIds.length > 0) {
        orphanWritten = await WrittenResult.findAll({ where: { quizId: { [Op.notIn]: validWrittenQIds } } });
        orphanedWrittenRes = await WrittenResult.destroy({ where: { quizId: { [Op.notIn]: validWrittenQIds } } });
      } else {
  ...
  server.delete("/api/admin/leaderboard-nuke", verifyToken, async (req, res) => {
    try {
      if (req.user.role !== 'admin') return res.status(403).send("Forbidden");
      ...
      if (validWrittenIds.length > 0) {
        orphanWritten = await WrittenResult.findAll({ where: { quizId: { [Op.notIn]: validWrittenIds } } });
      } else {
      ...
      let mcqWiped = 0, writtenWiped = 0;

      if (validMcqIds.length > 0) {
        mcqWiped = await McqResult.destroy({ where: { quizId: { [Op.notIn]: validMcqIds } } });
      } else {
        mcqWiped = await McqResult.destroy({ where: {} });
      }

      if (validWrittenIds.length > 0) {
        writtenWiped = await WrittenResult.destroy({ where: { quizId: { [Op.notIn]: validWrittenIds } } });
      } else {
  ...
  server.get('/api/admin/batches', verifyToken, async (req, res) => {
    try {
      ...
      const batches = await Batch.findAll({ attributes: ['name'], order: [['name', 'ASC']] });
      res.json(batches);
    } catch (error) {
  ...
  server.post('/api/admin/event-poster', verifyToken, uploadPoster.single('posterImage'), async (req, res) => {
    try {
      if (req.user.role !== 'admin') return res.status(403).json({ error: "Unauthorized" });
      ...
      if (batch && batch !== 'All') {
        ...
        if (!found) return res.status(400).json({ error: "Invalid batch name" });
        batchToUse = batch;
      }
  ...
  server.get('/api/admin/event-posters', verifyToken, async (req, res) => {
    try {
      if (req.user.role !== 'admin') return res.status(403).json({ error: "Unauthorized" });
      const posters = await EventPoster.findAll({ order: [['createdAt', 'DESC']] });
      res.json(posters);
    } catch (error) {
      res.status(500).json({ error: "Server error" });
    }
  ...
  server.put('/api/admin/event-poster/:id/toggle', verifyToken, async (req, res) => {
    try {
      if (req.user.role !== 'admin') return res.status(403).json({ error: "Unauthorized" });
      ...
      await EventPoster.update({ isActive }, { where: { id: req.params.id } });
